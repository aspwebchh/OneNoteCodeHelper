...
窗口高度固定：处理中的状态、思考摘录、执行步骤和结果都在下方卡片里显示，内容多了在卡片里滚动，窗口不会随内容忽高忽低。
...
- 请求走流式，窗口实时显示 AI 当前在做什么（思考中 / 输出中 / 已返回几段修改和最新一条改动说明）、
  思考原文的最后三行和已用时间。分了几批并发时，思考摘录只跟一批，不来回跳。思考原文只在窗口里显示，不写日志。
...
也可以输入「一级标题居中，正文统一 11 磅，关键结论加粗」。模型通过八个受限工具读取段落、调整草稿、提交修改，
界面显示执行进度，最终结果以插件回读 OneNote 后核验的数据为准。
处理中显示当前轮次、模型正在准备的工具，以及思考（或回复）原文的最后三行；「执行步骤」逐条列出调用过的工具和要点
...
- 除上面的错别字修正外，保留文字、段落数量、顺序、嵌套关系、列表、待办状态、链接和表格结构。不润色改写、不删空行、不移动图片；这些不属于本版 Agent 的工具范围。
...
- 「撤销本次」只在当前窗口中保留最近一次执行的已核验修改，修正过的错别字也一起还原。撤销会跳过后来又编辑过的段落。关闭窗口或启动下一次执行后不保留上一次撤销记录。
...
Services/
  ...
  LiveText.cs               Agent 窗口的思考摘录，以及边收边数 AI 已返回几段修改的 JSON 扫描
