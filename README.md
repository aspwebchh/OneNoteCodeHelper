...
| 功能 | 默认有「错别字修复」「排版优化」「错别字 + 排版」（前两项一次做完）三项，选项和提示词都来自配置文件，可以自己加 |
...
- 「排版优化」「错别字 + 排版」顺带删掉多余的空行：连续好几行空行只留一行，文本框（表格单元格）开头、结尾的空行删掉；
...
| `Functions/Function` | 「功能」下拉的选项，`name` 显示名、`Prompt` 提示词。`removeExtraBlankLines="true"` 表示顺带删多余的空行；不写时和同名的内置功能一致（「排版优化」「错别字 + 排版」默认开），写 `false` 关掉 |
...
所以以后新增的内置功能（比如「错别字 + 排版」）不会自动进旧的配置文件：自己在 `Functions` 里加一项，
