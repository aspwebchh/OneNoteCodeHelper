...
- 「排版优化」顺带删掉多余的空行：连续好几行空行只留一行，文本框（表格单元格）开头、结尾的空行删掉；
...
| `Functions/Function` | 「功能」下拉的选项，`name` 显示名、`Prompt` 提示词。`removeExtraBlankLines="true"` 表示顺带删多余的空行；不写时和同名的内置功能一致（「排版优化」默认开），写 `false` 关掉 |
