...
internal static class WindowPreview
{
    ...
    internal static int Render(string directory)
    {
        ...
        foreach (var width in new[] { 624, 504 })
        {
            var window = new AgentWindow(new NoAccess(), "preview", xml, AiConfigStore.Default, "example-model", "medium", IntPtr.Zero);
            var content = (FrameworkElement)window.Content;
            window.Content = null;
            var host = new System.Windows.Controls.Border { Background = new SolidColorBrush(Color.FromRgb(247, 248, 250)), Child = content };
            ...
            var path = Path.Combine(directory, "agent-window-" + width + ".png");
