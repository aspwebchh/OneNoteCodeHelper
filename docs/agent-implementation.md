...
3. `AgentRunner` 把模型原生 `tool_calls` 映射到六个本地函数，完整读取后才能修改段落。模型的文字声明不能触发写回。
...
| `get_pending_changes` | `snapshot_id`；返回草稿修订号、修改段落 ID、未完整读取的可编辑段落 ID 及保护计数 |
...
| `Views/AgentWindow.xaml(.cs)` | 范围、需求、进度、取消、结果、会话撤销 |
...
| `Services/Agent/AgentFormatting.cs` | 富文本解析与局部样式、CSS 归一化、预设、QuickStyleDef 管理 |
...
- HTTP 单次超时沿用 AI 配置；无新数据 60 秒终止。另有限制：总响应 600,000 字符、单工具参数 64,000 字符、JSON 最大嵌套 40 层。
...
| Agent 离线回归 | 48/48 通过，覆盖格式保留、Unicode、工具约束、协议分片、多轮、冲突、表格回存、取消、撤销和模拟 HTTP |
